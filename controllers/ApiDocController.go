...
func (self *ApiDocController) Index() {
	...
	groupInfo, err := group.GroupGetById(groupId)
	...
	//公共文档
	apiPublic, err := api.ApiPublicGetByIds(groupInfo.ApiPublicIds)
	...
	//接口
	apiMenu, _ := api.ApiTreeData(groupId)
...
func (self *ApiDocController) Public() {
	...
	apiPublic, _ := api.ApiPublicGetById(apiPublicId)
	self.Data["apiPublic"] = apiPublic
...
func (self *ApiDocController) Env() {
	...
	groupInfo, _ := group.GroupGetById(groupId)
	env, _ := env.EnvGetByIds(groupInfo.EnvIds)
	self.Data["env"] = env
...
func (self *ApiDocController) Code() {
	groupId, _ := self.GetInt("id", 0)
	groupInfo, _ := group.GroupGetById(groupId)
	code, _ := code.CodeGetByIds(groupInfo.CodeIds)
	self.Data["code"] = code
...
func (self *ApiDocController) ApiDetail() {
	...
	detail, _ := api.ApiFullDetailById(id)
