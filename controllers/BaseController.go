...
func (self *BaseController) Prepare() {
	...
	fmt.Println("self.controllerName", self.controllerName)
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			...
			if found && cheUser != nil { //从缓存取用户
				user = cheUser.(*admin.Admin)
			} else {
				user, err = admin.AdminGetById(userId)
...
func (self *BaseController) AdminAuth() {
	...
	if found && cheMen != nil { //从缓存取菜单
		...
		if self.userId != 1 {
			//普通管理员
			adminAuthIds, _ := role.RoleAuthGetByIds(self.user.RoleIds)
...
func groupLists() (gl []groupList) {
	...
	groupResult, _ := group.GroupGetList(1, 1000, groupFilters...)
...
func sourceLists() (sl []sourceList) {
	...
	sourceResult, _ := api.ApiSourceGetList(1, 1000, sourceFilters...)
...
func envLists() (sl []envList) {
	...
	envResult, _ := env.EnvGetList(1, 1000, envFilters...)
...
func templateLists() (sl []templateList) {
	...
	templateResult, _ := template.TemplateGetList(1, 1000, templateFilters...)
...
func codeLists() (sl []codeList) {
	...
	codeResult, _ := code.CodeGetList(1, 1000, codeFilters...)
...
func apiPublicLists() (sl []apiPublicList) {
	...
	apiPublicResult, _ := api.ApiPublicGetList(1, 1000, apiPublicFilters...)
