...
func (self *ApiController) Table() {
	...
	result, count := api.ApiDetailGetList(page, self.pageSize, filters...)
...
func (self *ApiController) AjaxDel() {
	...
	Api, _ := api.ApiDetailGetById(Api_id)
...
func (self *ApiController) AjaxChangeStatus() {
	...
	_, err := api.ApiChangeStatus(Api_ids, status)
...
func (self *ApiController) AjaxSave() {
	...
	if Api_id == 0 {
		...
		ApiDetail.Status = 1
		_, err := api.ApiDetailAdd(ApiDetail)
		if err != nil {
			self.ajaxMsg(err.Error(), MSG_ERR)
		}
		self.ajaxMsg("", MSG_OK)
	...
	//修改
	ApiDetail, _ := api.ApiDetailGetById(Api_id)
	ApiDetail.SourceId, _ = self.GetInt("source_id")
